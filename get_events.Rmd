...
knitr::opts_chunk$set(echo = T, echo=F, messages=F, warning=F, error = F, cache=T)
```
...
detailed.events <- lapply(states, function(state){
  ...
  tibble(name=name, gov.name=gov.name, gov.party=gov.party, status=status, still_open = open.detail, still_closed = closed.detail)
...
```{r}
saveRDS(graph.events, "data/orders.events.RDS")
saveRDS(detailed.events, "data/states.status.RDS")
