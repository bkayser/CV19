...
state.summary.g <- function(state.code='OR', overlay='Deaths') {
  ...
  scale.factor <- max(state.data$Cases.Diff5) / last.recorded.value
  ...
  ggplot(state.data) +
    ...
    geom_vline(data=orders,
               ...
               alpha=0.5,
               size=1) +
    scale_color_manual(values=c(close='#33FF33', open='#FF9999')) +
    geom_text(data=orders,
              aes(x=date,y=-0.04*last.recorded.value*scale.factor,
                  label=paste(Hmisc::capitalize(type),format(date, '%b %d'))),
              hjust='left',
              nudge_x=1) +
    xlab(NULL)  +
