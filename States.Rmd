...
```{r fig.width=5}
filter(cvdata.us, Date == last_date & !is.na(Population.Density)) %>%
...
filter(cvdata.us.by_state, Date == max(Date)) %>%
  arrange(Region, State.Code) %>%
  mutate(col = (row_number() %% 9), 
         row = (row_number() %/% 9),
         label_size = 0.06 * pmax(Cases, 15000),
