...
cvdata.us %>%
  ...
  select(Label, Cases.Per100K, Deaths.Per100K) %>%
...
hotspots <- cvdata.us %>%
  mutate(Label=paste0(County,", ", State.Code)) %>%
  filter(!is.na(Division) & Date == end_date & Population > 1000 & Cases > 50) %>%
  arrange(desc(Cases.Growth5)) %>%
  head(50) 

ggplot(hotspots) +
  aes(x=reorder(Label, Cases.Growth5), y=Cases.Growth5, fill=State.Code) +
