...
filter(cvdata.us.by_state, State.Code == 'OR') %>%
  ...
  ggtitle('Oregon Change in Confirmed Cases, Five Day Average',
          subtitle = str_c("Reported data through ", format(max(cvdata.us$Date), "%B %d, %Y"))) +
  ylab("Daily Increase") +
  geom_text(data=~ tail(.x, 1), aes(y=Deaths, label=paste0('Total Deaths: ',Deaths)), 
            #position='dodge', 
            color='red', 
            size=6, 
            nudge_y=-5, 
  ...
  xlab(NULL)  +
  geom_line(aes(y=Deaths), color='red') +
  coord_cartesian(xlim=c(ymd('2020-03-15'), end_date), ylim=c(0, 125)) +
  scale_x_date(breaks=date.breaks, date_labels = '%m/%d', date_minor_breaks='1 day') +
  theme_stata()
