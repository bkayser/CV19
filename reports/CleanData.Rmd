...
cvdata.us.by_state <- cvdata.us %>%
  ...
  mutate(State.Code = fct_inorder(as.factor(State.Code)),
         ...
         Testing.Rate.Total = total / Population,
         Testing.Positive.Rate.Total = positive / total) %>%
  calculate_differentials(c('total', 'positive', 'negative'), growth=F) %>%
  mutate(Testing.Rate.Weekly = 7 * total.Diff5 / Population,
         Testing.Positive.Rate = positive.Diff5 / total.Diff5) %>%
  add_estimated_recoveries() %>%
