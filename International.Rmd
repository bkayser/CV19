...
cvdata.i18n %>%
  ...
  geom_col() +
  scale_fill_gradient(low='#00FFAA', high='red', trans='log10') +
  geom_text(aes(label=round(Cases)), size=4, nudge_y=30) +
  coord_flip() +  
  xlab(NULL) +
...
cvdata.i18n %>%
  filter(Date == end_date, Cases.Growth3 > 0) %>%
  arrange(desc(Cases.Growth3)) %>%
  head(50) %>%
  ggplot() +
  aes(x=reorder(Key, Cases.Growth3), y=Cases.Growth3, fill=Cases.Growth3) +
  geom_col(show.legend=F) +
  scale_fill_gradient(low='#00FFAA', high='red', trans='log10') +
  geom_text(aes(label=percent(round(Cases.Growth3, 3))), size=4, nudge_y=0.008) +
  coord_flip() +  
  xlab(NULL) +
