...
# filter(state_data, Country == 'China, Hubei') %>% view()
...
d0 <- filter(cvdata.i18n, Cases > 100 & Cases.Growth3 > -0.01 & Country %in% c("China, Hubei", "Spain", "France", "Germany","Iran", "Italy", "Korea, South", "United Kingdom", "US")) %>%
  mutate(Country = as.character(Country)) %>%
  filter(!is.na(Population))

d1 <- group_by(d0, Country) %>%
  summarize(Cases.Growth3 = max(Cases.Growth3))

d2 <- semi_join(d0, d1, by=c('Country', 'Cases.Growth3')) %>%
  select(Country, Date)

start_dates <- d2$Date
names(start_dates) <- d2$Country
...
labels <- lapply(d2, tail, 1) %>% 
  bind_rows() %>%
  mutate(label = paste0(Country, " - ", format(Date, "%m/%d")))

# Let's predict the peak

extrapolate <- function(df) {
  c <- coef(lm(df, formula = Cases.Growth3 ~ Day))
  peak_day <- -c['(Intercept)']/c['Day']
  t0 <- min(df$Date)
  peak_date <- t0 + days(ceiling(peak_day))
}

# Rejoin the data
country_data <- bind_rows(d2)
# filter(country_data, Country == 'China, Hubei') %>% view()
...
g + aes(y=Cases.Growth3) +
  ylab("Daily Increase in Cases") +
#  geom_smooth(method='glm', formula= y ~ x, se=F, color='black', size=0.2) +
  scale_y_continuous(labels=percent) 
