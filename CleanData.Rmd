...
nyc <- filter(population_data, State == 'New York' & (CTYNAME %in% c('Bronx County', 'Kings County', 'Queens County', 'Richmond County', 'New York County'))) %>%
...
rm(regions, divisions, population_data, nyc)
...
cvdata.us <- left_join(cases, 
  ...
  left_join(read.csv('data/state_abbrev.csv') %>% select(State, State.Code=Code), by='State') %>% # Join State abbreviations
  filter(!is.na(Region)) %>%
