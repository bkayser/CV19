...
countries.filtered <- 
  filter(cvdata.i18n, Cases > 100 & Country %in% c("China, Hubei", "Spain", "France", "Germany","Iran", 
                                                   "Italy", "Korea, South", "United Kingdom", "US", 
                                                   "Russia", "India", "South Africa", "Iceland", "Belgium", "Ireland", "San Marino", "Sweden",
                                                   "Norway", "Denmark", "United Arab Emirates")) %>%
  mutate(Country = fct_relevel(Country, "China, Hubei", "Korea, South", 'Spain', 'Italy', 'US')) %>%
...
target_growth <- 
  filter(countries.filtered, Cases > 500 & Cases.Growth3 < 0.014 & Date > start_dates[Country]) %>%
  group_by(Country) %>%
...
display_countries <-  c("China, Hubei", "Spain", "France", "Germany","Iran", "Italy", "Korea, South", "United Kingdom", "US")
...
rm(filtered_data, country_data, countries.filtered, peak_growth, target_growth, start_dates, end_dates, countries.list, countries.pred_window,
   extrapolate.lm, predictions.country.list, predictions.country, predictions.country.target_day, display_countries)
```


```{r, fig.width=4, fig.height=3}

filtered_data <- lapply(prediction_data, function(data) { filter(data, Country %in% c("Germany", "India", "Russia", "US", "United Arab Emerates")) })

ggplot(filtered_data[['country_data']]) +
  aes(x=Day, y=Cases.Growth3, color=Country) +
  geom_line(show.legend = T) +
#  geom_line(data=filtered_data[['predictions.country']], size=0.5, linetype=3, show.legend=F) +
  scale_y_continuous(labels=percent) +
  theme(strip.text.x = element_text(face='bold', size=9)) +
  ggtitle(paste0("National Trends as of ",format(end_date, "%B %e, %Y")), subtitle="Plotted individually from the first day with more than 500 cases") +
  xlab("Days Since First 500 Cases") +
  ylab("Daily Increase in Cases") +
  geom_label(aes(label=percent(Cases.Growth3)), data=~tail(arrange(.x, Date), 4), nudge_y=0.01, show.legend = F) +
...
data <- lapply(predictions.country.list, tail, 1) %>% 
  bind_rows() %>%
  mutate(Days.From.Now=Date - today()) %>%
  select(Country, Date, Days.From.Now) %>%
  mutate(Date = format(Date, '%B %e'))
#write.csv(data, 'data/country-predictions-0411.csv')
kable(data)
...
data <- lapply(predictions.state.list, tail, 1) %>% 
  bind_rows() %>%
  mutate(Days.From.Now=Date - today()) %>%
  select(State, Date, Days.From.Now) %>%
  mutate(Date = format(Date, '%B %e')) 
# write.csv(data, 'data/state-predictions-0411.csv')
kable(data)
```
