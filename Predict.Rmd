...
peak_growth <- 
  semi_join(major_countries, 
            group_by(major_countries, Country) %>% summarize(Cases.Growth3 = max(Cases.Growth3)),
...
target_growth <- 
  filter(major_countries, Cases > 500 & Cases.Growth3 < 0.014 & Date > start_dates[Country]) %>%
  group_by(Country) %>%
