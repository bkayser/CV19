...
ggplot(country_data) +
  ...
  geom_line(data=predictions.country, color='black', size=0.5, linetype=3) +
  geom_vline(data=predictions.country.target_day, aes(xintercept = Day), color='darkgreen', size=0.5, alpha=0.5) +
  geom_label(data=predictions.country.target_day, aes(label = Label, x = Day), y = 0.25, color='darkgreen', size=3) +
  scale_y_continuous(labels=percent) +
  facet_wrap('Country') +
...
state_selection <-  c("New York", "Oregon", "California", "New Jersey", "Arizona", "Indiana",
                      "Washington", "South Dakota", "Florida")
training_window <- 20
